...
class Generator(nn.Module):
    ...
    def forward(self, x, texts_feature, boxes):
        ...
        input_comb_noise = torch.cat((x,input_feature),dim = 1)
        ...
        output = self.deconv_comb(com_feature)
...
class Discriminator(nn.Module):
    def __init__(self, cfg, feature_size = 1024):
        ...
        self.out_model = nn.Sequential(
            # state: (B,256,1,1)
            nn.Conv2d(256, 1, kernel_size=5, stride=1, padding=2, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x, texts_feature, boxes):
        ...
        ####################################################
        # Global pathway
        global_feature = self.conv_global(x).view(Batch_size,-1,1,1)
        global_add = global_feature + texts_feature

        comb_feature = local_feature.view(Batch_size,-1,1,1) + global_add

        output = self.out_model(comb_feature)
